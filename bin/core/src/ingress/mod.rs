...
pub async fn rebuild_ingress_cache() {
  ...
  // Pre-resolve each enabled instance's ingress server id
  let instance_server_ids: HashMap<String, Option<String>> = instances
    .iter()
    .filter(|i| i.config.enabled)
    .map(|instance| {
      let resolved = if !instance.config.server_id.is_empty() {
        if server_map.contains_key(&instance.config.server_id) {
          Some(instance.config.server_id.clone())
        } else {
          warn!(
            "Ingress instance '{}': server_id '{}' not found",
            instance.name, instance.config.server_id
          );
          None
        }
      } else {
      ...
      (instance.name.clone(), resolved)
    })
    .collect();

  // Single pass: iterate containers once, build both routes and Traefik labels per instance
  let mut instance_routes: HashMap<String, Vec<IngressRoute>> =
    HashMap::new();
  let mut instance_labels: HashMap<String, Vec<HashMap<String, String>>> =
    HashMap::new();
  ...
  for status in &server_status_list {
    ...
    for container in containers {
      ...
      // Skip containers for disabled/unknown instances
      let Some(ingress_server_id) =
        instance_server_ids.get(&instance_name)
      else {
        continue;
      };

      // Determine effective IP: use host.docker.internal when on same server as ingress
      let effective_ip = match ingress_server_id {
        Some(svr_id) if status.id == *svr_id => {
          "host.docker.internal".to_string()
        }
        _ => server_ip.clone(),
      };

      // Build Traefik labels with backend info
      let mut labels_with_backend =
        container.ingress_labels.clone();
      labels_with_backend
        .insert("__komodo_server_ip".to_string(), effective_ip.clone());
      ...
      instance_labels
        ...
        .or_default()
        .push(labels_with_backend);

      // Build IngressRoute entries from parsed labels
      let parsed_labels =
      ...
      for (router_name, router_labels) in &parsed_labels.routers {
        ...
        let backend_url = parsed_labels
          ...
          .map(String::from)
          .unwrap_or_else(|| {
            format!("http://{}:<unknown>", effective_ip)
          });
  ...
  // Build the final cache from collected data
  ...
  for instance in instances {
    ...
    new_cache.insert(
      ...
      CachedIngressState {
        routes,
        traefik_config: traefik_config::build(&labels),
      },
    );
  }
